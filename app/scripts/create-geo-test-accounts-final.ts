/**
 * Script final pour créer des comptes de test avec correspondance géographique aux proxies
 * Version corrigée avec le nom exact des colonnes dans la base de données
 */
 ...
 */
function getMajorCityForCountry(country: string): string {
  ...
  const cities = cityMap[country] || ['Unknown City'];
 ...
 */
function getRegionForCountry(country: string, city?: string): string {
  ...
  const countryRegions = regionMap[country] || regionMap['default'];
  
  if (city && countryRegions[city]) {
    return countryRegions[city];
  }
  
  return countryRegions['default'];
}
